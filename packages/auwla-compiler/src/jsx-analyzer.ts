...
function getJSXMemberExpressionName(expr: t.JSXMemberExpression): string {
  ...
  let property = '';
  if ((expr as any).property) {
    if (t.isIdentifier((expr as any).property)) property = ((expr as any).property as any).name;
    else property = (expr as any).property as string || '';
  }
...
export function expressionToString(expr: any): string {
  ...
  const property = expr && (t.isIdentifier(expr.property) ? expr.property.name : expressionToString(expr.property));

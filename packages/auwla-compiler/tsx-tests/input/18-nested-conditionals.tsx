// @page /nested-conditionals
// Complex nested conditional rendering patterns
...
const user = ref({
    id: 1,
    name: 'John Doe',
    role: 'user', // 'admin', 'moderator', 'user', 'guest'
    subscription: 'premium', // 'free', 'premium', 'enterprise'
                    ...
                    <div className="flex justify-between h-16">
                        <div className="flex items-center space-x-8">
                            ...
                            <div className="flex space-x-4">
                                ...
                                {$if((user.value.subscription === 'premium' || user.value.subscription === 'enterprise')) && (
                                    <a href="/analytics" className="text-gray-700">Analytics</a>
                                )}

                                {/* Admin navigation */}
                                {user.value.role === 'admin' && (
                                    <div className="flex space-x-4">
                                        <a href="/admin" className="text-red-600">Admin Panel</a>
                                        {user.value.permissions.includes('write') && (
                                            <a href="/admin/users" className="text-red-600">User Management</a>
                                        )}
                                    </div>
                                )}
                                ...
                                {$if((user.value.role === 'moderator' || user.value.role === 'admin')) && (
                                    <a href="/moderation" className="text-orange-600">Moderation</a>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="flex items-center space-x-4">
                            ...
                            {user.value.settings.notifications && (
                                <button className="relative">
                                    🔔
                                    {/* Notification badge for premium users */}
                ...
                {hasError.value ? (
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="bg-white rounded-lg shadow p-6">
                            ...
                            {!user.value.profile.isComplete && (
                                <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-4">
                                    <h3 className="text-yellow-800 font-medium">Complete Your Profile</h3>
                                    <div className="mt-2 space-y-2">
                                        {!user.value.profile.hasAvatar && (
                                            <p className="text-yellow-700">• Add a profile picture</p>
                                        )}
                                        {!user.value.profile.isVerified && (
                                            <p className="text-yellow-700">• Verify your email address</p>
                                        )}
                                    </div>
                                </div>
                            )}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            ...
                            {user.value.subscription !== 'free' ? (
                                <div className="bg-white rounded-lg shadow p-6">
                                    ...
                                    {features.value.advancedAnalytics ? (
                                ...
                                <div className="bg-gray-50 rounded-lg shadow p-6 opacity-75">
                                    <h3 className="font-medium mb-2">Advanced Analytics</h3>
                                    <p className="text-gray-600 text-sm mb-4">Detailed insights and reports</p>
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="flex items-center justify-between">
                                    <div>
                                        ...
                                        {features.value.customThemes && user.value.subscription !== 'free' && (
                                    ...
                                    {features.value.customThemes && user.value.subscription !== 'free' ? (
                                        <input
                                            type="checkbox"
                                            checked={user.value.settings.darkMode}
                                            onChange={(e) => user.value.settings.darkMode = e.target.checked}
                                        />
                                    ) : (
                                ...
                                <div className="flex items-center justify-between">
                                    ...
                                    <input
                                        type="checkbox"
                                        checked={user.value.settings.notifications}
                                        onChange={(e) => user.value.settings.notifications = e.target.checked}
                                    />
                                </div>
                                ...
                                {user.value.profile.isVerified && (
                                    <div className="flex items-center justify-between">
                                        ...
                                        <select
                                            value={user.value.settings.language}
                                            onChange={(e) => user.value.settings.language = e.target.value}

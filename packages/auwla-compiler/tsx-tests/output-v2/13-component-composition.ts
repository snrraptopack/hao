  ...
  return Component((ui: LayoutBuilder) => {
    ui.Div({ className: "p-4 border rounded" }, (ui: LayoutBuilder) => {
      ...
      ui.Div({ className: "flex gap-2 items-center" }, (ui: LayoutBuilder) => {
        ...
        ui.Span({ text: watch([count], () => `count.value`) as Ref<string> })
        ui.Button({ text: "+", on: { click: (e) => increment() } })
      })
      ui.P({ text: watch([globalCounter], () => `Global:${globalCounter.value}`) as Ref<string> })
    })
  })
}
...
const toggleTheme = () => {
  ...
  return Component((ui: LayoutBuilder) => {
    ui.Button({ text: watch([theme], () => `${theme.value === 'light' ? '🌙' : '☀️'}Toggle Theme`) as Ref<string>, className: `px-4 py-2 rounded ${theme.value === 'dark' ? 'bg-gray-800 text-white' : 'bg-gray-200'}`, on: { click: (e) => toggleTheme() } })
  })
}
...
const pageTitle: Ref<string> = ref('Component Composition Demo');
    ...
    ui.Div({ className: `min-h-screen p-8 ${theme.value === 'dark' ? 'bg-gray-900 text-white' : 'bg-white'}` }, (ui: LayoutBuilder) => {
      ui.Div({ className: "max-w-4xl mx-auto" }, (ui: LayoutBuilder) => {
        ui.H1({ text: watch([pageTitle], () => `pageTitle.value`) as Ref<string>, className: "text-3xl font-bold mb-6" })
        ui.Div({ className: "mb-6 flex gap-4 items-center" }, (ui: LayoutBuilder) => {
          ui.append(ThemeToggle({ theme: theme }))
          ui.Button({ text: watch([showCounters], () => `${showCounters.value ? 'Hide' : 'Show'}Counters`) as Ref<string>, className: "px-4 py-2 bg-blue-500 text-white rounded", on: { click: () => showCounters.value = !showCounters.value } })
          ui.Button({ text: watch([globalCounter], () => `Global +1 (${globalCounter.value})`) as Ref<string>, className: "px-4 py-2 bg-green-500 text-white rounded", on: { click: (e) => addGlobalCount() } })
        })

...
describe('renderToString', () => {
  ...
  it('resolves routed data and injects it into the component', async () => {
    ...
    expect(html).toContain('<script>window.__AUWLA_DATA__ = {"__loader:/posts/42":{"post":{"id":"42","title":"Server-rendered post"}},"remote:posts.getPost:42":{"id":"42","title":"Server-rendered post"}};</script>');
    expect(data).toEqual({
      '__loader:/posts/42': { post: { id: '42', title: 'Server-rendered post' } },
      'remote:posts.getPost:42': { id: '42', title: 'Server-rendered post' }
    });
  });

export type Stat = { mean: number; stddev: number; min: number; max: number; opsPerSec?: number };
...
export function measure(fn: () => void): number { const t0 = performance.now(); fn(); const t1 = performance.now(); return t1 - t0; }